    ...
    stages {
        ...
        stage('Build AMI') {
            steps {
                ...
                sh 'ls && cd packer && ls && export PACKER_LOG=1 && export PACKER_LOG_PATH=$WORKSPACE/packer.log && echo "packer log path:" $PACKER_LOG_PATH && /usr/bin/packer build master_linux.json &&  /usr/bin/packer build agent_linux.json'
                }
           }
        }
